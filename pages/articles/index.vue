...
<div class="posts" ref="animationBlock">
    ...
    <div class="layout-container">
        <div class="posts-row" v-for="(articles, i) in $chunk(posts.data, 3)" :key="i">
            <nuxt-link class="posts-item animation-item" v-for="article in articles" :key="article.id" :to="`/articles/${article.slug}`">
                <div class="posts-img">
                    <img :src="article.image" :alt="article.name[currentLang]">
                </div>
                <p class="posts-date">{{ $formatDate(article.created_at) }}</p>
                <h4 class="posts-heading">{{ article.name[currentLang] }}</h4>
                <p class="posts-text">
                  {{ article.description[currentLang] && article.description[currentLang].length > 150 ? article.description[currentLang].slice(0, 150) + '...' : article.description[currentLang] }}
                </p>
            </nuxt-link>
        </div>
