...
export default {
  ...
  beforeCreate() {
    this.componentId = () => import(`@/components/pages/${this.page.template}_inner`)
  },
  head() {
  ...
  async fetch({ store, params, route }) {
    const re = /\/.*\//gi
    const page = route.path.match(re).join('')
    await store.dispatch('pages/fetchPage', page)
    if (page === '/articles/') await store.dispatch('posts/fetchPost', params.page)
  }
}
</script>
